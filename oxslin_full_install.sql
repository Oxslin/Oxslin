-- ==========================================
-- INSTALACIÓN COMPLETA DE BASE DE DATOS PARA OXSLIN
-- Tablas, índices, triggers, RLS y funciones RPC oxslin_*
...
-- Nota: acceso de lectura para usuarios autenticados; operaciones sensibles vía service role.
...
-- ==========================================
-- RPC oxslin_* (con cleanup y funciones principales)
-- ==========================================
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
    -- Cleanup de funciones numix_* antiguas si existen
    BEGIN
        DROP FUNCTION IF EXISTS numix_get_limits(uuid) CASCADE;
        DROP FUNCTION IF EXISTS numix_get_limit(uuid,text) CASCADE;
        DROP FUNCTION IF EXISTS numix_update_limit(uuid,text,integer) CASCADE;
        DROP FUNCTION IF EXISTS numix_check_available(uuid,text,integer) CASCADE;
        DROP FUNCTION IF EXISTS numix_increment(uuid,integer,integer) CASCADE;
        DROP FUNCTION IF EXISTS numix_decrement(uuid,text,integer) CASCADE;
        DROP FUNCTION IF EXISTS numix_create_ticket(uuid,uuid,text,numeric,text,text,jsonb) CASCADE;
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
END
...
-- oxslin_get_limits
CREATE OR REPLACE FUNCTION oxslin_get_limits(
  p_event_id UUID
) RETURNS JSONB AS $$
BEGIN
...
-- oxslin_get_limit
CREATE OR REPLACE FUNCTION oxslin_get_limit(
  p_event_id UUID,
  p_number_range TEXT
) RETURNS JSONB AS $$
DECLARE
...
-- oxslin_update_limit
CREATE OR REPLACE FUNCTION oxslin_update_limit(
  p_event_id UUID,
  p_number_range TEXT,
  p_max_times INTEGER
) RETURNS JSONB AS $$
...
-- oxslin_check_available
CREATE OR REPLACE FUNCTION oxslin_check_available(
  p_event_id UUID,
  p_number_range TEXT,
...
-- oxslin_increment
CREATE OR REPLACE FUNCTION oxslin_increment(
  p_limit_id UUID,
  p_increment INTEGER,
  p_max_times INTEGER
) RETURNS BOOLEAN AS $$
...
-- oxslin_decrement
CREATE OR REPLACE FUNCTION oxslin_decrement(
  p_event_id UUID,
  p_number_range TEXT,
  p_decrement INTEGER
) RETURNS BOOLEAN AS $$
...
-- oxslin_create_ticket (simplificado; ajustar si se requiere más lógica)
CREATE OR REPLACE FUNCTION oxslin_create_ticket(
  p_id UUID,
  p_event_id UUID,
  p_client_name TEXT,
